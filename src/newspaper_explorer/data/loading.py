"""
ALTO XML parser for newspaper data.
Parses ALTO XML files in parallel and creates line-level DataFrames.
...
from newspaper_explorer.data.utils.alto_parser import ALTOParser
from newspaper_explorer.data.utils.mets_parser import METSParser
...
class DataLoader:
    ...
    def load_directory(
        ...
        with ProcessPoolExecutor(max_workers=self.max_workers) as executor:
            futures = {
                executor.submit(_parse_file_worker, str(fp), mets_cache): fp for fp in xml_files
            }
    ...
    def _build_mets_cache(self, alto_files: List[Path]) -> Dict[str, Dict]:
        ...
        with ProcessPoolExecutor(max_workers=self.max_workers) as executor:
            futures = {
                executor.submit(self._parse_mets_worker, str(mets_file)): mets_file
    ...
    @staticmethod
    def _parse_mets_worker(mets_path: str) -> tuple[str, Optional[Dict]]:
        """
    ...
    def load_files(
        ...
        with ProcessPoolExecutor(max_workers=self.max_workers) as executor:
            futures = {executor.submit(_parse_file_worker, str(fp)): fp for fp in filepaths}
