...
class ZenodoDownloader:
    ...
    def extract_part(self, part_name: str, fix_errors: bool = True) -> Path:
        ...
        # Check if we have dertagcopy structure or direct year folders
        source_path = None
        if dertagcopy_path.exists():
            source_path = dertagcopy_path
            logger.info(f"Found dertagcopy structure in {part_name}")
        else:
            # Check if temp_extract_path contains year directories directly (e.g., 1915-1917 archive)
            # Year directories should be 4-digit numbers (1900-2100)
            year_dirs = [
                d
                for d in natsorted(temp_extract_path.iterdir())
                if d.is_dir()
                and d.name.isdigit()
                and len(d.name) == 4
                and 1800 <= int(d.name) <= 2100
            ]
        ...
        if source_path:
            # Move year directories directly to raw/dataset_name/data_type/
            ...
            for year_dir in natsorted(source_path.iterdir()):
                if year_dir.is_dir() and year_dir.name.isdigit() and len(year_dir.name) == 4:
                    ...
                    if dest.exists():
                        ...
                        for item in natsorted(year_dir.iterdir()):
                            item_dest = dest / item.name
                            if not item_dest.exists():
                                shutil.move(str(item), str(item_dest))
                            else:
                                # Merge deeper if needed (for month directories)
                                if item.is_dir() and item_dest.is_dir():
