"""
Text Matcher - Universal text extraction from ALTO XML for any detection.

This module provides a unified approach to match any detection type (headlines,
captions, etc.) to OCR text content from ALTO XML based on bounding box overlap.
"""
...
class TextMatcher:
    """
    Universal text matcher for any detection type.

    Matches detection bounding boxes to ALTO XML text content by finding
    overlapping text lines/blocks.
    """
    ...
    def __init__(
        ...
        logger.info(
            f"TextMatcher initialized: overlap_threshold={overlap_threshold}, "
            f"min_confidence={min_confidence}"
        )

    def match_detections_to_text(
        self,
        ...
        """
        Match detections to OCR text from ALTO XML.
        ...
        Returns:
            List of Detection objects with matched text
        """
        if not detections:
        ...
        logger.debug(f"Matching {len(valid_detections)} detections to text")
        ...
        for detection in valid_detections:
            ...
            if matched_text:
                ...
                matched_count += 1
                logger.debug(
                    f"Matched {detection.class_name} ({detection.detection_id}): "
                    f"{matched_text[:50]}... (score: {match_score:.2f})"
                )

        logger.info(f"Matched text for {matched_count}/{len(valid_detections)} detections")
        return valid_detections
