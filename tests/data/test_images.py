...
def mock_config():
    """Mock configuration."""
    with patch("newspaper_explorer.data.download.images.get_config") as mock:
        config = Mock()
...
def test_image_downloader_initialization(mock_config, mock_source_config):
    ...
    with patch("newspaper_explorer.data.download.images.load_source_config") as mock_load:
...
def test_download_single_image(mock_config, mock_source_config, tmp_path):
    ...
    with patch("newspaper_explorer.data.download.images.load_source_config") as mock_load:
        ...
        # Disable validation for test with fake image data
        downloader = ImageDownloader(source_name="test_source", validate=False)
